...
# Load persisted values so widgets pick them up
_load_persisted_state()

# Default to test mode for development runs
...
with sidebar_col:
    ...
    if st.button("Generate DOCX from Current Preview", key="gen_current_docx", disabled=st.session_state.get('running', False)):
        ...
        try:
            ...
            st.session_state['last_parse_result'] = {'sections': parsed, 'roles': roles_p, 'skills': skills_p}

            # Indicate whether fallback would be used by _get_docx_bytes
            useful_sections = 0
            for sname in ('SUMMARY', 'EXPERIENCE', 'EDUCATION'):
                if parsed.get(sname):
                    useful_sections += 1
            st.session_state['last_parse_result']['fallback_needed'] = (useful_sections == 0)

            b = _get_docx_bytes(current_text)
            if b:
                st.session_state['current_debug_docx_bytes'] = b
                st.success(f"DOCX generated from current text (fallback_used={useful_sections==0}).")
            else:
    ...
    if st.session_state.get('last_parse_result'):
        try:
            st.markdown("**Last parse result (for current text):**")
            st.json(st.session_state.get('last_parse_result'))
        except Exception:
            st.write(st.session_state.get('last_parse_result'))
...
if run_clicked or run_requested:
    ...
    if not jd or not jd.strip() or not master or not master.strip():
        ...
        # Attempt to extract a trailing JSON blob (e.g. {"suggested_skills": [...]}) appended after the resume
        def _extract_json_blob(s: str):
            import json
            s = s or ""
            # Try to locate the last JSON-like structure and ensure it sits at the end
            last_positions = [s.rfind('{'), s.rfind('[')]
            starts = [p for p in last_positions if p != -1]
            if not starts:
                return None, s
            # Try candidates from right to left (prefer the last JSON fragment)
            for start_idx in sorted(starts, reverse=True):
                stack = []
                pairs = {'{': '}', '[': ']'}
                open_ch = s[start_idx]
                    ...
                    if s[k] == open_ch:
                        ...
                        if stack:
                            ...
                            if not stack:
                                ...
                                rest = s[end_idx:]
                                # Allow only whitespace and a possible trailing comma after the JSON
                                if rest.strip() not in ("", ","):
                                    break
                                try:
                                    parsed = json.loads(candidate)
                                    before = s[:start_idx].rstrip()
                                    # Remove a trailing comma before the JSON if present
                                    if before.endswith(','):
    ...
    if "result" in st.session_state:
        ...
        with st.expander("Recruiter Feedback", expanded=True):
            ...
            for w in fb["weaknesses"]:
                st.write("- " + w)
            # Show ATS breakdown if available
            try:
                if fb.get("ats_breakdown"):
                    st.markdown("**ATS score breakdown**")
                    bd = fb.get("ats_breakdown")
                    for k, v in bd.items():
                        st.write(f"- {k}: {v}")
            except Exception:
                pass
                ...
                for sec in ['PROFESSIONAL SUMMARY', 'SUMMARY', 'EXPERIENCE', 'SKILLS', 'EDUCATION']:
                    if sec in secs:
                        ...
                        for line in secs[sec]:
                            ...
                            if line.startswith('-'):
                                txt = line[1:].strip()
                                # Convert explicit {BOLD_START}/{BOLD_END} markers to markdown bold **...**
                                if '{BOLD_START}' in txt and '{BOLD_END}' in txt:
                                    s = txt
                                    md_parts = []
                                    while True:
                                        ...
                                        if si == -1:
                                            ...
                                            break
                                        if si > 0:
                                            md_parts.append(s[:si])
                                        s = s[si + len('{BOLD_START}') :]
                                        ei = s.find('{BOLD_END}')
                                        if ei == -1:
                                            md_parts.append(s)
                                            break
                                        md_parts.append('**' + s[:ei].strip() + '**')
                                        s = s[ei + len('{BOLD_END}') :]
                                    md_line = ''.join(md_parts).strip()
                                    parts.append(f"- {md_line}")
                                else:
                                    parts.append(f"- {txt}")
                            else:
                                ...
                                if sec == 'SKILLS' and ':' in line:
                                    ...
                                    parts.append(f"**{cat.strip()}:** {rest.strip()}")
                                else:
                                    parts.append(line)
                        parts.append('')
                return '\n'.join(parts)
